...
void AudioRenderer::run()
{
    ...
    LOG_INFO("AudioRenderer: QAudioSink started ({}Hz {}ch float)",
    ...
    while (!m_stop.loadRelaxed()) {
        ...
        if (isPaused != wasPaused) {
            if (isPaused) {
                ...
                m_lastEmitUs = m_sink->processedUSecs();
...
// 更新音频时钟
// ─────────────────────────────────────────────────────────────────────────────
void AudioRenderer::updateClock()
{
    if (!m_sink) return;

    // processedUSecs() 返回已实际送出到硬件的时间（微秒），
    // 不包含 QAudioSink 内部缓冲中尚未播出的数据，
    // 比"写入字节数 / 字节率"更准确。
    const qint64 clockUs = m_sink->processedUSecs();
