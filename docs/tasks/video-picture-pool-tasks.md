...
3% 的 F2 门槛。性能门禁结论为 **NO-GO**。后续产品决策明确要求实施 F2，因此以可控、
可诊断能力接入 SDK 默认路径，但不宣称性能收益。
...
F1 未通过收益门槛；后续产品决策覆盖了“是否实施”，但未覆盖性能结论。正式实现、开关、
诊断和真实媒体功能验证已完成，详见 `video-decoder-direct-rendering-f2-tasks.md` 与
`../performance/video-decoder-direct-rendering-f2-validation.md`。
